...
export default function Home() {
  ...
  const filteredPairs = search.length > 0
    ? pairs.filter(p => p.symbol.toLowerCase().includes(search.toLowerCase()) || p.name.toLowerCase().includes(search.toLowerCase()))
    : pairs
      ...
      <div style={S.root} onClick={() => setShowDropdown(false)}>
        <header style={S.header}>
          <div style={S.hInner}>
            ...
            {user && (
              <div style={S.hRight}>
                <div style={{...S.credits,cursor:'pointer'}} onClick={()=>setShowShop(true)} title="Comprar créditos">
                    ...
                    <div style={S.dropdown}>
                      {filteredPairs.slice(0,20).map(p=>(
                        <div key={p.symbol} onClick={()=>selectPair(p.symbol)}
                          style={{...S.dropdownItem,background:selectedPair===p.symbol?'#1a1428':'transparent'}}>
                      ...
                      {filteredPairs.length>20&&<div style={{padding:'8px 12px',fontSize:11,color:'#6060a0',textAlign:'center'}}>+{filteredPairs.length-20} — refine a busca</div>}
                    </div>
                  )}
                </div>
                {error&&<div style={{...S.errBox,marginTop:12}}>{error}</div>}
                {user.credits===0&&<div style={S.warnBox}>Créditos esgotados. <span style={{color:'#a78bfa',cursor:'pointer'}} onClick={()=>setShowShop(true)}>Comprar agora →</span></div>}
              </div>
                  ...
                  {sig.stochRsi&&(
                    <div style={{background:'#0f0f1a',border:'0.5px solid #1e1e30',borderRadius:6,padding:'10px 12px',marginBottom:10}}>
                      <div style={{display:'flex',alignItems:'center',marginBottom:8}}>
                        <span style={{fontSize:11,color:'#8080a0',textTransform:'uppercase',letterSpacing:'0.05em'}}>RSI Estocástico Lento</span>
                        <span style={{marginLeft:'auto',fontSize:11,fontWeight:600,color:sig.stochRsi.signal==='bullish'?'#4ade80':'#f87171',fontFamily:'monospace'}}>
                          {sig.stochRsi.signal==='bullish'?'↑ BULLISH':'↓ BEARISH'}
                        </span>
                      </div>
                      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:8}}>
                        <div style={{background:'#141424',borderRadius:6,padding:'8px 10px',border:'0.5px solid #1e1e30'}}>
                          <div style={{fontSize:10,color:'#6060a0',marginBottom:3}}>%K (lento)</div>
                          <div style={{fontFamily:'monospace',fontSize:13,fontWeight:600,color:sig.stochRsi.oversold?'#4ade80':sig.stochRsi.overbought?'#f87171':'#e2e2f0'}}>
                            {sig.stochRsi.k}
                            {sig.stochRsi.oversold&&<span style={{fontSize:10,color:'#4ade80',marginLeft:4}}>OVERSOLD</span>}
                            {sig.stochRsi.overbought&&<span style={{fontSize:10,color:'#f87171',marginLeft:4}}>OVERBOUGHT</span>}
                          </div>
                        </div>
                        <div style={{background:'#141424',borderRadius:6,padding:'8px 10px',border:'0.5px solid #1e1e30'}}>
                          <div style={{fontSize:10,color:'#6060a0',marginBottom:3}}>%D (sinal)</div>
                          <div style={{fontFamily:'monospace',fontSize:13,fontWeight:600,color:'#a78bfa'}}>{sig.stochRsi.d}</div>
                        </div>
                      </div>
                      <div style={{height:4,background:'#1e1e30',borderRadius:99,overflow:'hidden'}}>
                        <div style={{width:`${sig.stochRsi.k}%`,height:'100%',background:sig.stochRsi.oversold?'#4ade80':sig.stochRsi.overbought?'#f87171':'#a78bfa',borderRadius:99}}/>
                      </div>
                      <div style={{display:'flex',justifyContent:'space-between',fontSize:10,color:'#404060',marginTop:4}}>
                        <span>0 — Oversold</span><span>80-100 — Overbought</span>
                      </div>
                    </div>
                  )}
            ...
            <div style={{background:'#0a0a14',border:'1px solid #1a1a28',borderRadius:16,padding:28,maxWidth:420,width:'100%',maxHeight:'90vh',overflowY:'auto'}}
              ...
              {paySuccess?(
                <div style={{textAlign:'center',padding:'24px 0'}}>
                  ...
                  <div style={{color:'#8080a0',fontSize:13,marginTop:8}}>{selectedPkg?.credits} créditos adicionados.</div>
                </div>
              ):buyStep==='select'?(
                <>
